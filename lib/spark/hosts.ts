 ...
 *
 * The entry is the root, because on tentmaiker.com the product is the whole
 * site. Everything else keeps the shape it already had, one segment shorter.
 */
...
export const productPath = (pathname: string): string | null => {
  if (pathname === "/" || pathname === "") return SPARK_BASE;
