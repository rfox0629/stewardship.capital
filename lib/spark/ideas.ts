 ...
 * it costs, and the notes about it. Both planning surfaces ask exactly this
 * question, so both ask it here rather than each keeping a copy that drifts.
 *
 * Rows come through the reader's own session, so a guest never arrives here.
 */
...
export type MomentOption = { id: string; label: string };

export const gatherIdeas = async (
