-- The guest guide reads the card, when there is one.
--
-- weekend_guide() derived the guest schedule from every public row of the
-- working calendar. SHINE now has a written guest schedule on the engagement,
-- so the function prefers it: the card's days, times and words, with each line
-- able to borrow the guest copy of a calendar row so a meal still opens its
-- menu. An engagement with no card written keeps the derived reading.
--
-- And the weekend's verse, which the theme carries the way it carries the
-- tagline, and which the guide renders under the name.
...
begin
  update public.engagements
     set theme = jsonb_set(
           jsonb_set(coalesce(theme, '{}'::jsonb), '{copy,verse}',
             to_jsonb('Enlarge the place of your tent, stretch your tent curtains wide, do not hold back; lengthen your cords, strengthen your stakes.'::text), true),
           '{copy,verseRef}', to_jsonb('Isaiah 54:2'::text), true)
   where id = 'c4c371a6-6378-4f80-9d78-f9a6359ae8db';
  get diagnostics moved = row_count;
  if moved <> 1 then raise exception 'verse: % rows', moved; end if;
end $verse$;
